import { ActivityChangeReport, ActivityMetadata, EvidenceReference, ImpactReport, ProjectReport, RelationshipChangeReport } from "../lib/types";
...
export function ImpactInspector({ change, project, impact, onEvidence }: { change: ActivityChangeReport | null; project: ProjectReport | null; impact: ImpactReport | null; onEvidence: (reference: EvidenceReference) => void }) {
  ...
  const sourceMetadata = nodeMetadata(source) ?? Object.values(metadata).find((item) => item.activity_id === source && item.side === "after");
  const sourceName = sourceMetadata?.name;
  return <section className="inspector panel"><div className="inspector-heading"><div><span className="inspector-kicker">Selected activity</span><h2>{sourceName ?? source}</h2>{sourceName && <p className="activity-id-heading">{source}</p>}<p>{change.status} · confidence {(change.confidence * 100).toFixed(0)}%</p>{change.before_activity_id !== change.after_activity_id && <p className="identity-note">Reconciled identity: {change.before_activity_id ?? "—"} → {change.after_activity_id ?? "—"}</p>}</div><em className={"status " + `status-${change.status.toLowerCase().replaceAll("_", "-")}`}>{change.status}</em></div>
